...
const statisticsScore = async () => {
    const htmlRaw = await getHtmlRawOfPage(`${location.origin}${SITE_URL.studyResult}`)
    ...
    tableRows.filter((row) => row.children.length === 8).map((row) => {
        totalScoreGrade10 += parseFloat(row.children[5].textContent) * parseInt(row.children[2].textContent)
        countGrade4AVG[`${row.children[6].textContent}`]++
    })
    const grade10AVG = Math.round((totalScoreGrade10 / parseInt(totalCreditsEarned) * 100)) / 100
    ...
    document.getElementById('evaluate').innerHTML = getRankType(parseFloat(grade4AVG))
